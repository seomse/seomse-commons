...
import com.seomse.commons.data.StartEndImpl;
...
public class StringHighlightKeywordExample {
    public static void main(String[] args) {
        ...
        JSONArray split_start_ends =   obj.getJSONArray("split_start_ends");
        StartEndImpl[] splitStartEnds = new StartEndImpl[split_start_ends.length()];

        for (int i = 0; i <splitStartEnds.length ; i++) {
            JSONObject split_start_end = split_start_ends.getJSONObject(i);
            splitStartEnds[i] = new StartEndImpl(split_start_end.getInt("start"), split_start_end.getInt("end"));
        }
        ...
        String [] keywords = {"미생물", "유망", "기술"};
//        String [] keywords = {"음식물", "쓰레기" , "자원"};
        String highlight = StringHighlight.make(obj.getString("text"), tokens, tokenIndexes, splitStartEnds, keywords, "<en>", "</em>", 120);
